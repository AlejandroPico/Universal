...
let ws;const pending=new Map();let serial=0;const timeout=setTimeout(()=>{console.error('Browser verification timed out');preview.kill();chrome.kill();process.exit(1);},150000);
try{
 ...
 for(const [name,key] of [['Mercurio','mercury'],['Fobos','phobos'],['Deimos','deimos'],['Venus','venus-clouds']]){
